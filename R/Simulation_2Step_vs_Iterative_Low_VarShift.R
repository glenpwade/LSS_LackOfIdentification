...
if(FALSE)
{
    ...
    #
    # 1. T2000_Low_VarShift     TV_Pars: d0=0.5, d1=1.5, spd=2.3, loc=0.5    |    Garch_Pars: 0.10, 0.10, 0.80  (Matching LSS Specification)
...
Reps <- 5000
Tobs <- 2000
TVparScale <- c(0.5,1.5,log(10),0.5)
GARCHparScale <- c(0.10,0.10,0.80)
...
TVspec$pars["deltaN",1] = runif(1,1,2)     # 1.5
...
# Override the iteration count for testing:
# Reps <- 100    # Simulate 3000 estimations - set lower for debugging any parallel issues
...
results_2S = foreach(i=1:Reps, .combine = rbind, .inorder = TRUE, .packages = "MTVGARCH")%dopar%{
    ...
    mod$garchOptimcontrol$parscale <- GARCHparScale
    
    # 3. Run the 2-Step estimation
    mod$tvOptimcontrol$reltol <- iterRelTol    #Hack: This value is used as Threshold for the Iteration Convergence
    ...
    pars[1,] <- c(1,mod_2s@iterations,tvpars$delta0,tvpars$pars[1:3],garchpars$pars,as.numeric(!(mod_2s$Estimated$converged)) )
