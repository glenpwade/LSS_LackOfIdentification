...
GARCHspec$optimcontrol$parscale <- c(0.10,0.10,0.80)    # Keep the parameter scaling aligned with actual process
...
y_limit <- range(c(sqrt(TV@g), sqrt(GARCH@h)), na.rm = TRUE)
plot(sqrt(TV@g),type='l',ylim=y_limit,lwd=2,col="blue")
lines(sqrt(GARCH@h),col="grey",lwd=1)


modTVG <- tvgarch(TVspec,GARCHspec)
estCtrl <- list(calcSE=FALSE, verbose=TRUE, maxIter=10, fixStartPars=FALSE, startparAdjust=10)
mod <- estimateTVGARCH(e,modTVG,estCtrl)
summary(mod)
plot(mod)
