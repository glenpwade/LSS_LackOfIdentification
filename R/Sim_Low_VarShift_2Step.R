...
TVspec$optimcontrol$parscale <- c(0.5,1.5,log(10),0.5)
...
GARCHspec$optimcontrol$parscale <- c(0.10,0.10,0.80)    # Keep parScale = process params.
...
results_2s = foreach(i=1:Reps, .combine = rbind, .inorder = TRUE, .packages = "MTVGARCH")%dopar%{
    ...
    if (is.null(mod)) {
        ...
        # Return
        return(c(1,tvpars$delta0,tvpars$pars[1:3],garchpars$pars,(processLoglik - unname(mod$Estimated$value)),as.numeric(!(mod$Estimated$converged)),as.numeric(mod$Estimated$error) ))
    }
